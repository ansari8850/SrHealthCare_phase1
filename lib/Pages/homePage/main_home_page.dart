            ...
            child: Padding(
              ...
              child: SizedBox(
                ...
                child: ListView.builder(
                  itemCount: selectedLocationTypes.length,
                  scrollDirection: Axis.horizontal,
                  itemBuilder: (context, index) {
                    String tabName = selectedLocationTypes[index].name ?? '';
      ...
      if (post == null) {
        return SizedBox.shrink(); // Avoid displaying an empty item
                ...
                return Padding(
                  ...
                  child: GestureDetector(
                    onTap: () {
                     if(post!=null) Get.to(PostDetailPage(post: post ,));
                    },
                    child: Container(
                        ...
                        children: [
                              ...
                              CircleAvatar(
                                ...
                                child: CircleAvatar(
                                  ...
                                  child: AppCacheNetworkImage(
                                      width: Get.width,
                                      borderRadius: 50,
                                      imageUrl: post?.user?.photo?.url ?? '' , height: Get.height,),
                                ),
                              ),
                              const SizedBox(width: 10),
                              Expanded(
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              //     },
                              //     child: _buildAction(
                              //     post.isSaved==1? 'assets/myfeed/save.png'   :'assets/homepage/save.png',post.isSaved==1?"Saved": "Save")),
                              const SizedBox(
                                width: 15,
                              ),
                              GestureDetector(
...
class SaveButton extends StatefulWidget {
  const SaveButton({super.key, required this.isSaved, required this.postId , this.layoutType = Axis.horizontal});
  final bool isSaved;
  final int postId;
...
class SaveButtonState extends State<SaveButton> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () async {
        final result = isSaved
            ? await PostSaveAndUnsaveService().removeSavedPost(
                postId: widget.postId.toString(), context: context)
            : await PostSaveAndUnsaveService()
                .savePost(postId: widget.postId.toString(), context: context);
        if (result) {
          setState(() {
            isSaved = !isSaved;
          });
      ...
      child: widget.layoutType == Axis.horizontal ?Row(
        children: [
          CircleAvatar(
              radius: 15,
              backgroundColor: const Color(0xffBAF0F4).withOpacity(.4),
